...
│   │   ├── scene.py             # 屏幕场景状态机（云端归位 + 卡死/振荡脱困）
│   │   ├── app_goal_resolver.py # 自然语言目标 → 目标 Android 包名（app 边界硬约束）
│   │   ├── chat_title_helpers.py# 聊天标题识别 / 定位输入框 / 发送按钮匹配
│   │   ├── negotiation.py       # 协商机器人
│   │   ├── comm_log.py          # 上下行 + 原始 LLM 流量滚动日志
│   │   └── metrics.py           # 单任务指标采集（步数、LLM 调用、技能/缓存命中、耗时）
...
### 可观测性（`server/app/comm_log.py` / `metrics.py`）
...
### 聊天标题辅助（`server/app/chat_title_helpers.py`）
